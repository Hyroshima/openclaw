...
```
  ...
  <Tab title="Group policy and allowlists">
    ...
    `/approve <sender-id>` writes a grouped DM `allowFrom` entry with group `restricted`; `/approve <sender-id> friends` writes group `friends`. If the sender is already listed with a different group, the command updates the existing entry instead of adding a duplicate.
